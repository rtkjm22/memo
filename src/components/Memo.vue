...
<section class="alert alert-primary">
  ...
  <div class="form-control-group mt-3">
    <label for="content" class="col-12 text-left h5">Memo</label>
  ...
  setup(props) {
    ...
    const page_items = computed(function() {
      if (data.find_flg) {
        ...
        rec.forEach(el => {
          console.log('hoge');
          if (el.title.toLowerCase().indexOf(data.title.toLowerCase()) >= 0) {
            arr.push(el)
          }
        })
        return arr
    ...
    const page = computed({
      ...
      set: (p) => {
        let pg = p > (data.store.state.memo.length - 1) / data.num_per_page ? Math.ceil((data.store.state.memo.length -1) / data.num_per_page) -1 : p
        pg = pg < 0 ? 0 : pg
